...
class Food extends InventoryItem {
  ...
  */
  ...
  get numOfExoticIngredients() {
    return this.ingredients.filter(ingredient => ingredient.type == IngredientType.EXOTIC).quantity || 0;
  }

  get cost() {
    return Math.pow(
      this.numOfOrdinaryIngredients + this.numOfExoticIngredients,
      (1 + this.numOfExoticIngredients) * ((11 + this.numOfEquipment) / 10) * 1.2
    );
  }
}
...
var dish = new Food(
  "Suck my Dish",
  [new Ingredient(IngredientType.EGG, 2), new Ingredient(IngredientType.NUTS)],
